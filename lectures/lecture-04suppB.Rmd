...
description: |
  This is the beginning of my scientific-looking article.
#slug: distill2020
...
You can change the shape of points in `ggplot`:

```{r point-shapes, echo = TRUE, eval = FALSE}
df <- expand.grid(x = 1:3, y = 1:2) %>% 
  mutate(id = factor(1:n()))

ggplot(df, aes(x, y, shape = id)) + 
  geom_point(size = 4) + 
  theme_void() +
  guides(shape = FALSE) + 
...
<aside>
```{r point-shapes, fig.height = 2, fig.width = 2}
```
</aside>


This #TidyTuesday plot originally by Maia Pelletier is beautiful! 😻
...
Thank you to RStudio for contributing to open-source projects and the many tools that we use.
